...
func (r *SetupHandler) Setup(g *gin.Engine, c schema.Module) {
	...
	g.DELETE("api/v1/accounts/fcm-token/:token", m.CheckToken(), ac.DeleteToken)
	...
	g.POST("api/v1/foods/:foodId/measurement-histories", m.CheckToken(), mhc.CreateHistory)
	...
	g.POST("api/v1/foods/measurement-histories/app", m.CheckToken(), mhc.CreateHistoryFromApp)
